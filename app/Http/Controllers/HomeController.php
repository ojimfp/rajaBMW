...
class HomeController extends Controller
{
    ...
    public static function homeData()
    {
        ...
        //         ->join('dim_date', 'fact_order.Date_ID', '=', 'dim_date.Date_ID')
        //         ->selectRaw('COUNT(DISTINCT fact_order.Order_ID)/COUNT(DISTINCT fact_order.Customer_ID) as clv')
        //         ->where('dim_date.Year', 2017)
        //         ->get();
        ...
        $retention = \DB::table('fact_order')
                ...
                //     ->join('dim_date', 'fact_order.Date_ID', '=', 'dim_date.Date_ID')
                    ...
                    ->orWhere('dim_customer.Customer_Category', '=', 'Returning Customer')
                //     ->where('dim_date.Year', 2017)
                    ->get();

        // Customer Churn
        // $churn = \DB::table('fact_order')
        //         ->join('dim_customer', 'fact_order.Customer_ID', '=', 'dim_customer.Customer_ID')
        //         ->join('dim_date', 'fact_order.Date_ID', '=', 'dim_date.Date_ID')
        //         ->selectRaw('COUNT(DISTINCT fact_order.Customer_ID) as reten')
        //         ->where('dim_customer.Customer_Category', '=', 'Loyal Customer')
        ...
        return view('dashboard', compact('prodSold', 'netProfit', 'margin', 'aov', 'totalCust', 'aoq', 'aof', 'retention'));
    }
}
